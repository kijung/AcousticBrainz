...
if __name__ == "__main__":
    ...
    weights = joblib.load(specific + '_weights.txt')
    data = dict()
    test_prediction = dict()
    genres = list(locations.keys())
    genres.sort()
    ...
    for item1 in lst:
        for item2 in lst:
            ...
            path = './' + specific + '/' + item1+item2 + '.pkl'
            test_features = joblib.load(path)
            subset_keys = test_features.keys()
            test_features = test_features.values()
            keys += subset_keys
            for genre in genres:
                path = './' + specific + '/' + specific + '_randomforest_classifiers_' + str(locations[genre]) + '.pkl'
                ...
                for p in percent:
                    ...
                    test_prediction[p][genre] += list(clf[p].predict(selective_features))
    ...
    for p in percent:
        writeToTsv(specific + '_' + str(p)+'_train_test_RFC.tsv', test_prediction[p], dict(), keys)